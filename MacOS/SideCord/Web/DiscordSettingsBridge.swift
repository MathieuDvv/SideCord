...
struct SideCordMarketplaceSettingsSnapshot: Codable, Equatable, Sendable {
    let identifier: String
    let name: String
    let version: String
    let summary: String
...
struct SideCordSettingsSnapshot: Codable, Equatable, Sendable {
    ...
    var marketplaceConfigured = false
    var marketplaceRefreshing = false
    var marketplaceUsingCache = false
    var marketplaceError: String?
    var marketplacePlugins: [SideCordMarketplaceSettingsSnapshot] = []
}
...
extension DiscordCSSComposer {
    ...
    static func settingsBridgeUserScriptSource(snapshot: SideCordSettingsSnapshot) -> String {
        ...
        (() => {
          ...
          if (previous?.version === 10 && typeof previous.update === "function") {
          ...
          const state = {
            ...
            pluginListHost: null,
            pluginListSignature: "",
            marketplaceListHost: null,
            marketplaceListSignature: "",
            pluginSearchQuery: "",
            update: null,
            open: null,
            dispose: null
          ...
          const pageHTML = () => `
            <style>
              ...
              .sc-library-tools { display:flex; align-items:center; gap:10px; margin:12px 0 4px; }
              .sc-library-search { flex:1; min-width:0; border:1px solid rgb(255 255 255 / 20%);
                border-radius:6px; padding:10px 12px; color:inherit; background:rgb(255 255 255 / 7%); font:inherit; }
              html[data-sidecord-resolved-color-scheme="light"] .sc-library-search {
                border-color:rgb(31 35 40 / 24%); background:rgb(31 35 40 / 6%); }
              .sc-plugin-identity { color:#b8bdc7; font-size:12px; font-family:ui-monospace,monospace; }
              html[data-sidecord-resolved-color-scheme="light"] .sc-plugin-identity { color:#4b5563; }
              .sc-plugin-tags { color:#9aa4ff; font-size:12px; }
              html[data-sidecord-resolved-color-scheme="light"] .sc-plugin-tags { color:#343d9f; }
              .sc-library-status { color:#b8bdc7; font-size:13px; margin:8px 0 0; }
              html[data-sidecord-resolved-color-scheme="light"] .sc-library-status { color:#4b5563; }
            ...
            <div class="sc-wrap">
              ...
              <section class="sc-section" data-sidecord-page="theme"><h2>Appearance</h2>
                ${row("Theme","Curated SideCord surface palette.",select("visualTheme",[["systemGlass","Mica"],["discord","Discord"],["oled","OLED"],["soft","Soft"]]))}
              ...
              <section class="sc-section" data-sidecord-page="plugins"><h2>Plugin library</h2>
                <div class="sc-library-tools">
                  <input class="sc-library-search" type="search" placeholder="Search plugins, publishers, or categories" data-sidecord-plugin-search>
                  <button class="sc-button sc-secondary" type="button" data-sidecord-action="refreshMarketplace">Refresh</button>
                </div>
                <p class="sc-library-status" data-sidecord-marketplace-status></p>
                <div class="sc-plugin-list" data-sidecord-marketplace-list></div>
                <h2 style="margin-top:30px">Installed plugins</h2><div class="sc-footer">
          ...
          const renderMarketplaceList = () => {
            const host = safeQuery(state.page, "[data-sidecord-marketplace-list]");
            const status = safeQuery(state.page, "[data-sidecord-marketplace-status]");
            const search = safeQuery(state.page, "[data-sidecord-plugin-search]");
            if (!host || !status || !search) return;
            if (search.value !== state.pluginSearchQuery && search !== document.activeElement) {
              search.value = state.pluginSearchQuery;
            }
            const allEntries = Array.isArray(state.snapshot.marketplacePlugins)
              ? state.snapshot.marketplacePlugins : [];
            const query = state.pluginSearchQuery.trim().toLowerCase();
            const entries = !query ? allEntries : allEntries.filter(entry => [
              entry.identifier, entry.name, entry.summary, entry.publisher,
              entry.repository, ...(entry.categories || [])
            ].some(value => String(value || "").toLowerCase().includes(query)));
            if (!state.snapshot.marketplaceConfigured) {
              status.textContent = "This build does not contain a signed plugin-catalog URL and verification key.";
            } else if (state.snapshot.marketplaceRefreshing) {
              status.textContent = "Refreshing the signed catalog…";
            } else if (state.snapshot.marketplaceError) {
              status.textContent = String(state.snapshot.marketplaceError);
            } else {
              status.textContent = `${entries.length} of ${allEntries.length} plugins` +
                (state.snapshot.marketplaceUsingCache ? " · offline cache" : "");
            }
            let signature = "";
            try { signature = JSON.stringify([entries, query, state.snapshot.marketplaceRefreshing]); }
            catch (_) {}
            if (state.marketplaceListHost === host && state.marketplaceListSignature === signature) return;
            state.marketplaceListHost = host;
            state.marketplaceListSignature = signature;
            host.replaceChildren();
            if (!entries.length) {
              const empty = document.createElement("small");
              empty.textContent = query ? "No plugins match your search." : "No catalog entries are available.";
            ...
            for (const entry of entries) {
              ...
              name.textContent = String(entry.name || entry.identifier || "Plugin");
              const identity = document.createElement("span");
              identity.className = "sc-plugin-identity";
              identity.textContent = String(entry.repository || entry.identifier || "");
              const publisher = document.createElement("small");
              publisher.textContent = entry.publisher
                ? `Published by @${entry.publisher}${entry.verifiedPublisher ? " · Verified" : ""} · ${entry.version}`
                : `Publisher identity unavailable · ${entry.version}`;
              const summary = document.createElement("span");
              summary.textContent = String(entry.summary || "");
              description.append(name, identity, publisher, summary);
              const action = document.createElement("button");
              action.type = "button";
              action.className = "sc-button";
              const updateAvailable = !!entry.updateAvailable;
              action.textContent = entry.blockedReason ? "Blocked" :
                updateAvailable ? "Update" : entry.installedVersion ? "Installed" : "Install";
              action.disabled = !!entry.blockedReason || (!!entry.installedVersion && !updateAvailable);
              action.addEventListener("click", event => {
                event.stopPropagation();
                post({ type: "settingsAction",
                  action: updateAvailable ? "updateMarketplacePlugin" : "installMarketplacePlugin",
                  identifier: String(entry.identifier || "") });
              });
              head.append(description, action);
              rowNode.appendChild(head);
              if ((entry.categories || []).length) {
                const tags = document.createElement("span");
                tags.className = "sc-plugin-tags";
                tags.textContent = entry.categories.map(value => `#${value}`).join("  ");
                rowNode.appendChild(tags);
              }
              ...
              const hosts = (entry.networkHosts || []).join(", ");
              permission.textContent = entry.blockedReason ||
                `Permissions: ${permissions}${hosts ? ` · Hosts: ${hosts}` : ""}`;
          ...
          const renderPluginList = () => {
            ...
            try {
              signature = JSON.stringify(plugins.map(plugin => {
                ...
                return [
                  ...
                  String(plugin.repository || ""), String(plugin.publisher || ""),
                  String(plugin.availableUpdateVersion || ""),
                  panel ? [
            ...
            for (const plugin of plugins) {
              ...
              version.textContent = plugin.publisher
                ? `Published by @${plugin.publisher}${plugin.verifiedPublisher ? " · Verified" : ""} · ${plugin.version}`
                : `Author: ${String(plugin.author || "Unknown")} · ${String(plugin.version || "")}`;
              description.append(name);
              if (plugin.repository) {
                const identity = document.createElement("span");
                identity.className = "sc-plugin-identity";
                identity.textContent = String(plugin.repository);
              ...
              if (plugin.availableUpdateVersion) {
                const update = document.createElement("button");
                update.type = "button";
                update.className = "sc-button";
                update.textContent = `Update to ${plugin.availableUpdateVersion}`;
                update.addEventListener("click", event => {
                  event.stopPropagation();
                  post({ type: "settingsAction", action: "updateMarketplacePlugin",
                    identifier: String(plugin.identifier || "") });
                });
                removeActions.appendChild(update);
              }
          ...
          const bindPageInteractions = node => {
            ...
            node.addEventListener("input", event => {
              const search = event.target?.closest?.("[data-sidecord-plugin-search]");
              if (search) {
                state.pluginSearchQuery = String(search.value || "");
                state.marketplaceListSignature = "";
                renderMarketplaceList();
                return;
              }
              const input = event.target?.closest?.('input[type="range"][data-sidecord-key]');
