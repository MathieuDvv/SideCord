...
final class DiscordWebController: NSObject, ObservableObject {
    ...
    private func observeSettings() {
        ...
        Publishers.MergeMany([
            pluginManager.$installed.dropFirst().map { _ in () }.eraseToAnyPublisher(),
            pluginManager.$enabledIdentifiers.dropFirst().map { _ in () }.eraseToAnyPublisher(),
            pluginManager.$catalog.dropFirst().map { _ in () }.eraseToAnyPublisher(),
            pluginManager.$catalogIsCached.dropFirst().map { _ in () }.eraseToAnyPublisher(),
            pluginManager.$marketplaceError.dropFirst().map { _ in () }.eraseToAnyPublisher(),
            pluginManager.$isRefreshingMarketplace.dropFirst().map { _ in () }.eraseToAnyPublisher()
        ])
        .debounce(for: .milliseconds(40), scheduler: RunLoop.main)
        .sink { [weak self] _ in self?.refreshCSS() }
        .store(in: &settingsCancellables)
        ...
        SideCordSettingsSnapshot(
            ...
            plugins: pluginManager.installed.map { plugin in
                ...
                return SideCordPluginSettingsSnapshot(
                    ...
                    repository: plugin.marketplaceMetadata?.repository.pathComponents
                        .dropFirst()
                        .joined(separator: "/"),
                    publisher: plugin.marketplaceMetadata?.publisher,
                    verifiedPublisher: plugin.marketplaceMetadata?.verifiedPublisher ?? false,
                    availableUpdateVersion: pluginManager.availableUpdate(for: plugin)?.version,
                    webPanel: webPanel
                )
            },
            marketplaceConfigured: pluginManager.isMarketplaceConfigured,
            marketplaceRefreshing: pluginManager.isRefreshingMarketplace,
            marketplaceUsingCache: pluginManager.catalogIsCached,
            marketplaceError: pluginManager.marketplaceError,
            marketplacePlugins: (pluginManager.catalog?.plugins ?? []).map { entry in
                SideCordMarketplaceSettingsSnapshot(
                    identifier: entry.identifier,
                    name: entry.name,
                    version: entry.version,
                    summary: entry.summary,
                    repository: entry.repositoryDisplayName,
                    publisher: entry.publisher,
                    verifiedPublisher: entry.verifiedPublisher,
                    categories: entry.categories,
                    permissions: entry.permissions,
                    networkHosts: entry.networkHosts,
                    installedVersion: pluginManager.installed.first {
                        $0.id == entry.identifier
                    }?.manifest.version,
                    updateAvailable: pluginManager.installed.first {
                        $0.id == entry.identifier
                    }.map {
                        SideCordPluginManager.isVersion(
                            entry.version,
                            newerThan: $0.manifest.version
                        )
                    } ?? false,
                    blockedReason: pluginManager.blockReason(for: entry)
                )
            }
        )
    }
    ...
    private func handleSettingsAction(_ payload: [String: Any]) {
        ...
        case "refreshMarketplace":
            Task { [weak self] in
                await self?.pluginManager.refreshMarketplace()
            }
        case "installMarketplacePlugin", "updateMarketplacePlugin":
            guard let identifier = payload["identifier"] as? String,
                  identifier.count <= 128,
                  let entry = pluginManager.catalog?.plugins.first(where: {
                      $0.identifier == identifier
                  })
            else { return }
            Task { [weak self] in
                await self?.prepareMarketplaceEntryFromSettings(entry)
            }
    ...
    private func prepareMarketplaceEntryFromSettings(
        _ entry: SideCordMarketplaceEntry
    ) async {
        do {
            let plan = try await pluginManager.prepareMarketplaceInstallation(entry)
            let alert = NSAlert()
            alert.messageText = plan.isUpdate
                ? "Update \(entry.name) to \(entry.version)?"
                : "Install \(entry.name)?"
            var details: [String] = []
            if let repository = entry.repositoryDisplayName {
                details.append(repository)
            }
            details.append(entry.publisherDisplayName)
            details.append(plan.confirmationSummary)
            alert.informativeText = details.joined(separator: "\n\n")
            alert.addButton(withTitle: plan.isUpdate ? "Update" : "Install")
            ...
            downloadError = "Couldn’t install plugin: \(error.localizedDescription)"
            refreshCSS()
        }
    }
