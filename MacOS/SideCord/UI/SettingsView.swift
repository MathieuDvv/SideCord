...
struct SettingsView: View {
    ...
    @State private var pendingMarketplacePlan: SideCordMarketplaceInstallPlan?
    @State private var showingMarketplaceConfirmation = false
    @State private var marketplaceOperationIdentifier: String?
    ...
    var body: some View {
        ...
        .confirmationDialog(
            pendingMarketplacePlan?.isUpdate == true
                ? "Update \(pendingMarketplacePlan?.entry.name ?? "plugin")?"
                : "Install \(pendingMarketplacePlan?.entry.name ?? "plugin")?",
            isPresented: $showingMarketplaceConfirmation
        ) {
            Button(pendingMarketplacePlan?.isUpdate == true ? "Update Plugin" : "Install Plugin") {
                commitPendingMarketplacePlan()
            }
            Button("Cancel", role: .cancel) { pendingMarketplacePlan = nil }
        } message: {
            Text(pendingMarketplacePlan?.confirmationSummary ?? "Review this plugin before continuing.")
        }
    }
    ...
    private var pluginsPage: some View {
        VStack(spacing: 18) {
            SettingsGlassCard(
                ...
                subtitle: "Discover signed catalog entries sourced from public GitHub repositories.",
                symbol: "shippingbox.fill",
                tint: .indigo
            ) {
                if pluginManager.isMarketplaceConfigured {
                    TextField("Search plugins, publishers, or categories", text: $marketplaceSearchText)
                        .textFieldStyle(.roundedBorder)

                    HStack {
                        Text(pluginManager.catalog == nil
                             ? "Refresh to load the curated catalog."
                             : "\(filteredMarketplaceEntries.count) of \(pluginManager.catalog?.plugins.count ?? 0) packages")
                            .foregroundStyle(.secondary)
                        if pluginManager.catalogIsCached {
                    ...
                    ForEach(filteredMarketplaceEntries) { entry in
                        ...
                        HStack(alignment: .top, spacing: 12) {
                            VStack(alignment: .leading, spacing: 3) {
                                ...
                                if let repositoryName = entry.repositoryDisplayName {
                                    Text(repositoryName)
                                        .font(.caption.monospaced())
                                        .foregroundStyle(.secondary)
                                }
                                HStack(spacing: 5) {
                                    Text(entry.publisherDisplayName)
                                    if entry.verifiedPublisher {
                                ...
                                if !entry.categories.isEmpty {
                                    Text(entry.categories.map { "#\($0)" }.joined(separator: "  "))
                                        .font(.caption)
                                        .foregroundStyle(.indigo)
                                }
                                if let repository = entry.repository {
                                    Link("View repository", destination: repository)
                                        .font(.caption)
                                }
                            }
                            Spacer()
                            if marketplaceOperationIdentifier == entry.identifier {
                                ProgressView().controlSize(.small)
                            } else {
                                Button(marketplaceActionTitle(for: entry)) {
                                    prepareMarketplaceEntry(entry)
                                }
                                .disabled(marketplaceActionIsDisabled(for: entry))
                            }
                        }
                    }

                    if pluginManager.catalog != nil, filteredMarketplaceEntries.isEmpty {
                        ContentUnavailableView.search(text: marketplaceSearchText)
                    }
                } else {
                    Label(
        ...
        .task {
            if pluginManager.isMarketplaceConfigured, pluginManager.catalog == nil {
                await pluginManager.refreshMarketplace()
            }
        }
    }

    private var filteredMarketplaceEntries: [SideCordMarketplaceEntry] {
        let entries = pluginManager.catalog?.plugins ?? []
        let query = marketplaceSearchText.trimmingCharacters(in: .whitespacesAndNewlines)
            .lowercased()
        ...
        return entries.filter { entry in
            [
                ...
                entry.author,
                entry.publisher ?? "",
                entry.repositoryDisplayName ?? "",
                entry.categories.joined(separator: " "),
                entry.permissions.joined(separator: " "),
                entry.networkHosts.joined(separator: " ")
            ].contains { $0.lowercased().contains(query) }
        }
    }

    private func marketplaceActionTitle(for entry: SideCordMarketplaceEntry) -> String {
        if pluginManager.blockReason(for: entry) != nil { return "Blocked" }
        guard let installed = pluginManager.installed.first(where: { $0.id == entry.identifier })
        else { return "Install" }
        return SideCordPluginManager.isVersion(entry.version, newerThan: installed.manifest.version)
            ? "Update"
            : "Installed"
    }

    private func marketplaceActionIsDisabled(for entry: SideCordMarketplaceEntry) -> Bool {
        let title = marketplaceActionTitle(for: entry)
        return title == "Blocked" || title == "Installed"
    }
        ...
        VStack(alignment: .leading, spacing: 11) {
            HStack(alignment: .top) {
                VStack(alignment: .leading, spacing: 3) {
                    ...
                    if let marketplace = plugin.marketplaceMetadata {
                        Text(marketplace.repository.pathComponents.dropFirst().joined(separator: "/"))
                            .font(.caption.monospaced())
                            .foregroundStyle(.secondary)
                        HStack(spacing: 5) {
                            Text("Published by @\(marketplace.publisher)")
                            if marketplace.verifiedPublisher {
                        ...
                        Text("Author: \(plugin.manifest.author) · \(plugin.manifest.version) · \(plugin.source.rawValue.capitalized)")
                            .font(.caption)
                            .foregroundStyle(.secondary)
                    }
            ...
            HStack {
                ...
                if let update = pluginManager.availableUpdate(for: plugin) {
                    Button("Update to \(update.version)") {
                        prepareMarketplaceEntry(update)
                    }
                }
                Spacer()
    ...
    private func prepareMarketplaceEntry(_ entry: SideCordMarketplaceEntry) {
        marketplaceOperationIdentifier = entry.identifier
        Task {
            defer { marketplaceOperationIdentifier = nil }
            do {
                pendingMarketplacePlan = try await pluginManager.prepareMarketplaceInstallation(entry)
                showingMarketplaceConfirmation = true
            }
            catch {
                presentedError = PresentedSettingsError(
                    title: "Couldn’t prepare plugin",
                    message: error.localizedDescription
                )
            }
        }
    ...
    private func commitPendingMarketplacePlan() {
        defer { pendingMarketplacePlan = nil }
        guard let plan = pendingMarketplacePlan else { return }
        do {
            _ = try pluginManager.commitMarketplaceInstallation(plan)
        } catch {
            presentedError = PresentedSettingsError(
                title: plan.isUpdate ? "Couldn’t update plugin" : "Couldn’t install plugin",
                message: error.localizedDescription
            )
        }
    }
